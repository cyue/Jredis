...
public class BinaryClient extends Connection {
    ...
    public void sentinel(byte[] key) {
    	sendCommand(SENTINEL, key);
    }
    
    public void sentinel(byte[] key, byte[] option) {
    	sendCommand(SENTINEL, key, option);
    }
    
    public void sentinel(byte[] key, byte[] host, int port) {
    	sendCommand(SENTINEL, key, host, toByteArray(port));
    }
}
